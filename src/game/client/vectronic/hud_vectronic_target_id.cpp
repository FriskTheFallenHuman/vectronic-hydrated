...
void CTargetID::Paint()
{
	...
	if ( iEntIndex )
	{
		...
		if ( IsPlayerIndex( iEntIndex ) )
		{
			...
			if ( rHGameRules()->IsDeathmatch() == false && pPlayer->InSameTeam( pLocalPlayer ) )
			{
				printFormatString = "#Playerid_Friend";
				bShowHealth = true;
			}
			else
			{
				printFormatString = "#Playerid_FrienNoHealth";
			}
		...
		if ( sIDString[0] )
		{
			...
			if ( !GetTargetInScreenSpace( pLocalPlayer, xpos, ypos ) ) 
				return;
