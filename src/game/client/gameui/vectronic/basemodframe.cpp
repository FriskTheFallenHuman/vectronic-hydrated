...
{
	GenericConfirmation* confirmation = 
		static_cast< GenericConfirmation* >( CBaseModPanel::GetSingleton().OpenWindow( WT_GENERICCONFIRMATION, caller, false ) );

	GenericConfirmation::Data_t data;

	data.pWindowTitle = title;
	data.pMessageText = messge;
	data.bOkButtonEnabled = okbutton;
	data.pfnOkCallback = command;
	data.bCancelButtonEnabled = cancelbutton;
...
//=============================================================================
void CBaseModFrame::MakeGenericDialog( const char* title, const char* messge, bool okbutton, Callback_t command, bool cancelbutton )
{
	MakeGenericDialog( title, messge, okbutton, command, cancelbutton, NULL );
}
...
int CBaseModFrame::DrawSmearBackground( int x, int y, int wide, int tall )
{
...
int CBaseModFrame::DrawSmearBackgroundFade( int x0, int y0, int x1, int y1, float f1, float f2 )
{
	int wide = x1 - x0;
	int tall = y1 - y0;
...
int CBaseModFrame::DrawBlackBackground( int x, int y, int wide, int tall )
{
	CMatRenderContextPtr pRenderContext( materials );
